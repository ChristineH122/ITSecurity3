...
export default class Store {
  ...
  public async isAdminFromID(sessionId: string): Promise<boolean> {
    if (!sessionId) {
      return null;
    }
    let user = await this.handleError(this.userRepo.findOne({uuid: sessionId}));
    if(user){
      return user.role ? user.role.name == "admin" : null;
    }
    else{
      return null;
    }
